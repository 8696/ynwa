...
└── app/                      前端应用根
    ...
    │   ├── config.js         站点身份、DB_URL、PAGE_SIZE、PINNED_MAX、缓存键、页脚、「更多」卡文案
...
| **置顶 / 取消置顶（文章）** | `articles[].pinned`（正整数，越小越靠前）；上限 `PINNED_MAX`（默认 3，`core/config.js`） | 键必留。未置顶写 `null`。超过 3 篇写了正整数时运行时只认 rank 最小的 3 篇 |
| **置顶 / 取消置顶（作品）** | `works[].pinned`，规则同上；身份用 `href` | 不要把 `pinned` 写进「更多」卡（那张在 `config.js`）。作品无 date，同 rank 按原数组下标 |
...
| **加纯函数（日期、搜索、URL、置顶排序）** | `core/utils.js` | 无 JSX、无 `import`/`export`；挂到全局 `function foo()` |
...
| `core/config.js` | **仅**站点身份与运行常量（含 `DB_URL` / `PAGE_SIZE` / `PINNED_MAX` / `CACHE_*`） | 经典脚本，`var` 全局；无 JSX；列表数据不进这里 |
...
| `Search` | `/search` |
| `Works` | `/works`（置顶抽到最前；网格与首页 `#works` 共用 `.bento`） |
| `NotFound` | `*` |
...
- **贴在 `--accent` 上的字用 `--on-accent`**，不要用 `--ink`。深色 accent（ink / 蓝 / 咖啡）日子会隐身（`/topics` 分类计数徽标栽过）。置顶徽标 `.chip--pin` 已走这对变量
- 区块大标题用 `.section-title`（更大用 `--lg` / `--display`）
- 按钮用 `.btn`，小按钮叠加 `.btn--sm`
- 正文只走 `.markdown-body`，不要再加一套 `.prose`
- 断点：作品双列 `640px`（`.bento` 用 CSS grid，从左到右再换行；**不要** `column-count`），其余 `768px`
...
`id` 用 UUID（macOS：`uuidgen`），**不要用标题拼音、不要改已发布文章的 id**。`date` 用 `YYYY-MM-DD HH:MM`。无封面时 `cover` 留 `""`，未置顶 `pinned` 留 `null`，不要删这两个键。不要把正文粘进 JSON。数组末项不要逗号。
...
- `"pinned"`：正整数越小越靠前；不置顶写 `null`。全站最多 `PINNED_MAX`（默认 3）篇生效；超额的只当普通文。取消置顶改回 `null`，不要删键
- `categories[0]` 决定从该文进详情时顶栏高亮哪个分类（把最能代表该文的分类放第一位）
- `file` 用站内绝对路径（前导 `/`）或 `https://...`
- 搜索只扫 `title` 和 `summary`，不扫正文、不扫标签名
- JSON 数组书写顺序无所谓，进内存后按 `date` 倒序；首页 / 分类 / 标签列表再把有效置顶抽到最前
...
1. 首页列表能看到，且按日期排在该出现的位置（`pinned` 为正整数的会先于日期出现在列表顶部，最多 3 篇；详情页标题上方也有「置顶」徽标）
...
| 标题、摘要、分类、标签、日期、封面、置顶 | 改 `articles[]` 对应字段；**不要改 `id`**。置顶写 `pinned` 正整数，取消改回 `null` |
...
3. 注意：
   ...
   - `"pinned"`：正整数越小越靠前，不置顶写 `null`（不要删键）。全站最多 `PINNED_MAX`（默认 3）条，与文章共用上限、各自竞选
   - 首页最多 `SITE_WORKS_MAX`（默认 4）张卡，**末张永远是「更多」**（文案在 `core/config.js` 的 `SITE_WORKS_MORE`），所以作品只切前 `max - 1` 条（置顶会先被抽到前面再切）；完整列表在 `/works`
   - `works[]` 顺序 = 未置顶时的展示顺序（不按日期排）
...
| 置顶超额不浮动 / 没徽标 | 全站文章、全站作品各自最多 `PINNED_MAX` 条生效；落选的当普通项。漏写 `pinned` 键或写成 `0`/`false` 也不置顶 |
| 作品卡先填满左列再填右列 | `.bento` 误用了 `column-count`。应保持 CSS grid，从左到右再换行 |
...
6. 若动过置顶：首页 / 分类 / `/works` 的顺序与徽标对得上；搜索只打徽标、不改命中顺序
7. 若动过作品网格：宽屏是左右左右，不是左列填完再填右列
8. 若动过 `db.json`：文件仍是合法 JSON
9. **改完必须核对 `AGENTS.md` 和 `README.md`**（硬约束第 10 条）：约定/清单进本文件，路由/字段/架构进 `README.md`；确认不用改也要说清理由
10. 若增删了 `components/*` 或 `pages/*`：`index.html` 的 `LOCAL_ASSETS.components` / `.pages` 与磁盘文件一一对应（每项 `js`/`css` 都存在），无漏挂、无指向已删文件
